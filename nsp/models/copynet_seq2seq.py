...
class CopyNetSeq2Seq(Model):
    ...
    def __init__(self,
        ...
        self._denotation_metric = denotation_metric
    ...
    def forward(self,  # type: ignore
        ...
        if not self.training:
            ...
            if target_tokens:
                ...
                if self._denotation_metric is not None and metadata[0]["split"]=='test': #calc only for test split
                    predicted_tokens = self._get_predicted_tokens(output_dict["predictions"],
                                                                  metadata,
                                                                  n_best=self._beam_search.beam_size
                                                                  )
                    self._denotation_metric(predicted_tokens,  # type: ignore
                                             [x["target_tokens"] for x in metadata], [x["variables"] for x in metadata], [x["index"] for x in metadata],
                                            self._domain)
    ...
    def get_metrics(self, reset: bool = False) -> Dict[str, float]:
        ...
        if not self.training:
            ...
            if self._denotation_metric is not None:
                all_metrics.update(self._denotation_metric.get_metric(reset=reset))  # type: ignore
        return all_metrics